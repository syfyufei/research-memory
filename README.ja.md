...
1. **研究サイクルは、あなたが考えるよりも必ず長くなります。**
2. **同時に開くリポジトリは、あなたが考えるよりも必ず多くなります。**
3. **次回戻ってきたときには、必ず自分が何をしていたか忘れています。**

vibe-codingの日々では、これが最も一般的な光景かもしれません：

> 3ヶ月間放置されていたリポジトリを開き、
> `git log` を上から下までスクロールし、
> notebook、スクリプト、論文ドラフトはすべてそこにあるが、
> 心には一つの質問しかない：
> **「前回はどこまで進めたのだっけ？」**

さらに厄介なのは：
**あなたは忘れてしまい、LLMも忘れてしまいます。**
新しい対話を始めると、LLMは現在の数画面のコンテキストしか知りませんが、
過去数ヶ月のプロジェクトの苦悩、遠回り、ブレークスルーについては全く理解していません。

`research-memory`は、vibe-codingのプロセスにおいて、**真に「長期的な」プロジェクトレベルの記憶**の層を追加することを目的としています：

- 汎用RAGはせず、文献の記憶を助けません；
- 忠実に記録します：このプロジェクトでのあなたとあなたのコパイロットが
  **何をしたか、どのようにしたか、なぜそのようにしたか、次に何をするか**を。

言い換えれば：

> 「未来のあなた + 現在のLLM」
> が「過去のあなたたち」がたどった道を引き継げるようにし、
> 毎回空のダイアログボックスからやり直すのを避けます。


## 機能概要（TL;DR）

`research-memory`は、学術研究プロジェクト向けに設計されたClaude Codeスキル/Pythonツールで、3つのことを提供します：

1. **セッションブートストラップ：`research_memory_bootstrap`**
   - `memory/`ディレクトリからプル：
     - プロジェクト概要（研究問題、仮説、データソース...）
     - 最近のN件のdevlogエントリ
     - 現在のTODOリスト
   - 自動的に「最近の進捗 + 作業計画の提案」を生成
...
2. **セッション記録：`research_memory_log_session`**
   - 研究作業の構造化記録：
     ...
     - `experiments.csv`：実験設定ごとに1行（hypothesis/dataset/model/metrics/notes...）
     - `decisions.md`：重要な決定とその根拠/代替案
     - `todos.md`：新しいTODOを追加し、完了したものを`[x]`としてマーク

3. **履歴クエリ：`research_memory_query_history`**
   - キーワード + シンプルなフィルタリング（日付 / フェーズ / タイプ）で検索：
     - `devlog.md`
     - `decisions.md`
     - `experiments.csv`
   - 要約 + キースニペットを返す。

**すべてローカルテキストファイル（Markdown + CSV）ベース、Gitフレンドリー、手動編集可能、外部サービス依存なし。**
...
もしあなたが複数の大規模プロジェクト間を頻繁に行き来し、
毎回「昨日の自分が何を考えていたか」を思い出すのに半日費やしたくないなら、
研究リポジトリに`research-memory`をインストールしてみてください—
Claude Codeを**プロジェクト全体の歴史を真に記憶している**パートナーにし、
現在の対話しか記憶していないチャットオブジェクトではなくしてください。
